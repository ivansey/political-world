...
if($userr[name] == '') {
die("<div class='block'>Кого ты ищешь?Такого нет</div><div class='a'><a href=edit_uzver.php>Назад</a></div>");
}

if($user[priv] <= $userr[priv]) {
die('<div class="block">Нельзя!</div><div class="a"><a href=edit_uzver.php>Назад</a></div>');
}
if(isset($_POST['post'])){
$name = $_POST['name'];
$about = $_POST['about'];
...
if($user[priv] >= 3) {
$priv = $_POST['priv'];
$queryy = $conn->query("UPDATE users SET name = '$name', about = '$about', tag = '$tag', money = $money, gold = $gold, energy = $energy, lvl = $lvl, exp = $exp, party = $party, region = $region, gover = $gover, priv = $priv WHERE id = $userr[id]");
} else {
...
echo "<input type='submit' name='post' value='Изменить'>";
echo "</form>";
echo "</div>";
break;
case 3:
...
?>
<div class='a'><a href=edit_uzver.php>Назад</a></div>
