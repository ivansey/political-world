...
if ($gover_sql == 0) {
    echo '<div class="block-up">Вы не находитесь в каком либо государстве</div>';
}

if ($par == 1) {
    echo '<div class="block-up">Выборы в парламент<br>';
    ...
    while ($i <= $par_sum) {
        ...
        echo '<div class="block-info-up">' . $party['name'] . ' | Голосов: ' . $par_sql['vote'] . ' | ' . $procent . '</div>';
        echo '<div class="a-down"><a href="vote.php?id=' . $par_sql['id'] . '&type=1">Проголосовать</a></div><br>';
        $i++;
    }
echo '</div>';
} elseif ($lead == 1) {
    echo '<div class="block-up">Выборы лидера государства<br>';
...
//        $ = $conn->query("SELECT * FROM `party` WHERE `id` = " . $par_sql['id_party'])->fetch();
        ...
        echo '<div class="block-info-up">' . name($user_info) . ' | Голосов: ' . $user_sql['vote'] . ' | ' . $procent . '</div>';
