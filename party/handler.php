...
try {
    if ($user['gold'] >= 100) {
        $query = $conn->prepare('INSERT INTO party (name, about, leader, tag) VALUES(:name, :about, :leader, :tag)');
 ...
 $party = $conn->query("SELECT * FROM `party` WHERE `leader` = " . $user[id] . " ")->fetch();
     $conn->query("UPDATE `users` SET `party` = " . $party[id] . ", `gold` = `gold` - 100, `tag` = '".$tag."' WHERE `id` = ".$user[id]." AND tag is null");
